...
## 3.5. Evaluando la performance del modelo
...
```{r fc matriz de confusion, echo = FALSE, cache = TRUE}
...
## 3.6. Mejorando la performance del modelo
...
Al iterar con diferentes valores de *k*, se observa que el mejor resultado se obtiene con un *k* igual a 11, 12 ó 13 (resulta extraño que se recomiende 114 para un dataset de este tamaño).
```{r}
k_alternativos_z %>% 
...
De la misma manera, se crea una tabla de valores alternativos para *k*, pero con la normalización min-max.
...
Es válido realizar el mismo gráfico (con distintos valores de *k*), pero con los datos normalizados con min-max. A continuación se muestra la representación ilustrando las lineas de ambos modelos, con diferente método de normalización.
...
k_alternativos %>%
        ...
        geom_vline(xintercept = k_alternativos$k_value[k_alternativos$accuracy_fc == max(k_alternativos$accuracy_fc)], alpha = 0.3) +
        theme_minimal() +
        scale_x_continuous(
                breaks = c(11, 50, 100)
        ) +
        labs(
                ...
                title = "¿Cuál es el mejor valor de k?",
                x = "k",
                y = "Accuracy"
        )
```
Aquí se aprecia que los valores de *k* con los que se logra el mejor performance son coincidentes en ambos métodos de normalización y que se encuentran entre 11 y 13 (vecinos cercanos).
