...
El análisis no explica qué variables son las que más inciden en el resultado. Como ya se ha aclarado al comienzo, una de las debilidades del algoritmo k-NN es que no produce un modelo, propiamente dicho, por lo que no es posible entender cómo los atributos se relacionan con el resultado. 

Sin embargo, a pesar de que el cómo es una caja negra, este estudio puede llegar a ser de utilidad para algunos actores involucrados en el comercio exterior argentino que, desde la perspectiva de su negocio o misión, les resulta conveniente identificar anticipadamente si una empresa logrará exportar o no, por ejemplo, para ofrecer sus servicios.

La investigación parte de algunos supuestos y, a partir del mismo, nacen algunas incógnitas y oportunidades de mejoras que pueden merecer un análisis más profundo. En primer lugar, "los datos superan a los algoritmos", que es una de las principales máximas de la ciencia de datos (*data beats algorithm*), resume la idea de que la calidad del modelo depende de la calidad, cantidad y variedad de los datos utilizados para construirlo. Para testear si los datos utilizados son los mejores posibles, se debería probar el mismo modelo, empleando otra data. A continuación se sugieren tres alternativas a explorar.
